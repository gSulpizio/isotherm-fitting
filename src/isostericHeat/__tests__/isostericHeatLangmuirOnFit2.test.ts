...
function data1(wholeData: any) {
  let data1 = {
    x: wholeData.p1.map((x: number) => x / Math.max(...wholeData.p1)),
    y: wholeData.n,
  };
  let options1 = {
    damping: 10e-2,
...
function data2(wholeData: any) {
  let data2 = {
    x: wholeData.p2.map((x: number) => x / Math.max(...wholeData.p2)),
    y: wholeData.n,
  };
  let options2 = {
    damping: 10e-2,
...
function data3(wholeData: any) {
  let data3 = {
    x: wholeData.p3.map((x: number) => x / Math.max(...wholeData.p3)),
    y: wholeData.n,
  };
  let options3 = {
    damping: 10e-2,
